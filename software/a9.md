...
-  Use SL7 [container](/tutorials/containers) via Singularity for job submission:
   ...
   > ```
- Test your script interactively inside the [container](/tutorials/containers):
    ```bash  
    singularity shell --shell /usr/bin/csh -B /direct -B /star -B /afs -B /gpfs -B /sdcc/lustre02 /cvmfs/star.sdcc.bnl.gov/containers/rhic_sl7.sif
    ```
    I suggest to create an [alias](https://www.geeksforgeeks.org/linux-unix/alias-command-in-linux-with-examples/) for it
    - In the container:
    > ```bash
...
```
However, for faster usage either one coul either:
- [mount](https://star-juniors.github.io/software/remote-development.html#mounting-sdcc-on-your-laptop)/copy `.root` files locally
- use [VSCode extension + `ssh`](https://star-juniors.github.io/software/vs-code-tunnel) 
- use [NoMachine](https://star-juniors.github.io/software/remote-development.html#no-machine) (VNC server)
