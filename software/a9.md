...
- Update `~/.cshrc` and `~/.login` for NFSv4:
   ...
   setenv GROUP_DIR /star/nfs4/AFS/star/group
    ```
-  Use new a9 `starsub` nodes when [`ssh`](/software/ssh): `starsub01 - starsub07`

-  Use SL7 [container](/software/containers) via Singularity for job submission:
   ...
   > ```
- Test your script interactively inside the [container](/software/containers):
    ```bash  
