...
1. Follow steps 1-2 [here](/software/ssh/transfer_files_rcf#mount-the-remote-file-system-windows).
...
3. Open Visual Studio and get any extensions you might want. These are useful, especially if you plan on running code locally (__Disclaimer__; These are not all compilers or debuggers, and in many cases, you need to install a separate compiler. If you want to use many of these extensions properly, follow all of their directions). My recommendations are the [C/C++ Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools-extension-pack), [Jupyter](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter),and [Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python).
4. Follow steps 3-6 [here](/software/ssh/transfer_files_rcf#mount-the-remote-file-system-windows).
...
Make sure to save any edited files, doing so will save those changes on SDCC as well. Also keep in mind, if you want to run your code, you will still need to `ssh` into SDCC since the STAR libraries are not local. Use your preferred method of login from [here](/software/ssh/rcf_generic_login).
